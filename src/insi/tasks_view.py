...
def render_tasks_panel(
    ...
    def render_material(material, progress) -> None:
    ...
    def render_task(task_document, progress) -> None:
        ...
        if (
            ...
            saved_order = saved_activity_value(answer_key, progress=progress)
            order = (
                saved_order
    ...
    def task_panel(title, icon, document, renderer) -> None:
        with ui.expansion(title, icon=icon).classes("w-full") as expansion:
    ...
    if material_tasks:
        ui.separator()
        ui.label("Weitere Aufgaben").classes("text-xl font-bold text-primary")
        ui.label("Freie Antworten und interaktive Zuordnungsaufgaben.").classes("text-grey-7")
        for material in material_tasks:
            task_panel(material.title, "description", material, render_material)

    current_paradigm = None
    for document in visible_tasks:
        if document.name not in trainable_names:
            continue
        if document.paradigm != current_paradigm:
            current_paradigm = document.paradigm
            ui.separator()
            ui.label(
                "Imperative Aufgaben" if current_paradigm == "imperativ"
                else "Objektorientierte Aufgaben"
            ).classes("text-xl font-bold text-primary")
        task_panel(get_exercise(document.name).title, "task_alt", document, render_task)
